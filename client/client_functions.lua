...
TryToFish = function()
    QBCore.Functions.TriggerCallback('elite-fishing:GetItemData', function(count)
        if IsPedSwimming(cachedData["ped"]) then return exports['mythic_notify']:DoLongHudText ('error', 'Няма как да плуваш и да ловиш!') end 
        if IsPedInAnyVehicle(cachedData["ped"]) then return exports['mythic_notify']:DoLongHudText ('error', 'Излез от ППС ,за да започнеш да ловиш!') end 
        if count ~= nil then
            if count == 0 then
                exports['mythic_notify']:DoLongHudText ('error', 'Нещо май ти липсва!')
            else
                ...
                if waterValidated then
                    ...
                    exports['mythic_notify']:DoCustomHudText ('error', 'ТРЯБВА ДА СИ НА ЛОДКА ИЛИ В БЛИЗОСТ ДО ВОДА!', 7500)
                end
            end
        end
...
CastBait = function(rodHandle, castLocation)
    ...
    if not HasFishingBait() then
        exports['mythic_notify']:DoLongHudText ('error', 'Нямаш стръв!')

        isFishing = false
        return DeleteEntity(rodHandle)
    end
    exports['mythic_notify']:DoLongHudText ('inform', 'НАТИСНИ [G] ,ЗА ДА ЗАПОЧНЕШ')
    while not IsControlJustPressed(0, 47) do
        Wait(5)

        if GetGameTimer() - startedCasting > 5000 then
            exports['mythic_notify']:DoLongHudText ('inform', 'Прибра си въдицата!')
    ...
    exports['mythic_notify']:DoLongHudText ('inform', 'Пий една бира и почакай...')
    ...
    if caughtFish then
        TriggerServerEvent("elite-fishing:receiveFish", castLocation, function(received) end)
        TriggerServerEvent('hud:server:RelieveStress', 1)
    else
        exports['mythic_notify']:DoLongHudText ('error', 'Рибата се изплъзна от въдицата...')
    end
...
TryToCatchFish = function()
    ...
    exports['mythic_notify']:DoHudText('inform', 'НАТИСНИ [E] В ЗЕЛЕНАТА ЗОНА', { ['background-color'] = '#1ca800', ['color'] = '#000000' })
    while true do
        Wait(5)
...
IsInWater = function()
    ...
    exports['mythic_notify']:DoLongHudText ('inform', 'В ОТКРИТО МОРЕ ДЕБНАТ ИНТЕРЕСНИ СЪЩЕСТВА...')
    

    while GetGameTimer() - startedCheck < 5000 do
        Wait(0)
    end
...
SellFish = function()
    QBCore.Functions.TriggerCallback('elite-fishing:GetItemData', function(count)
        ...
        TriggerServerEvent("elite-fishing:sellFish", function(sold, fishesSold) end)
    end)
end
...
RegisterNetEvent('elite-fishing:client:spawnFish', function(args)
	local time = 20000
	local args = 1
	if args == 1 then 
